...
function TodoPage() {
  ...
  useEffect(() => {
    const getTodoList = async () => {
      ...
      // console.log(res) // object가 옴
      setTodoList(res.data.data); // res의 data의 data에 초기 설명해준 todoList가 [] 빈배열로 있음
    };
  ...
  const handleAddTodo = (title, content) => {
    ...
    // 관심사 분리의 장점? TodoApi.addTodo를 만들어 놓으면 여러 원하는 컴포넌트에서 가져다 쓸수 있기 때문에 재사용성 측면에서 편리
