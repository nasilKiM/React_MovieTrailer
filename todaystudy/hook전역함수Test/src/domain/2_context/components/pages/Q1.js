...
const ContextQ1Page = () => {
  ...
  // console.log(state); // {isModalState: false, setIsModalState: ƒ}   => state 변수 안에 ModalContext에서 만든 createContext 저장소 안의 value가 담겨 있는 것.
  // useContext()는 괄호 안에 있는 저장소의 값을 사용하는 방법!

  const { isModalState } = state;

  return (
    // 3. Provider로 감싸기
    // <ModalContext.Provider value={{ isModalOpen, setIsModalOpen }}>
    <div>
      {isModalState && <ContextQ1Modal />}
      <h2>문제 2 - 1</h2>
      <ContextQ1Detail />
